...
```bash
npm test        # 75 deterministic tests, no key needed
...
**Deterministic (`npm test`, 75 tests, no key) — the dependable gate.** One test
...
highest-value group: the extraction layer fed what a 70B open model actually returns —
markdown fences, prose wrappers, invented option strings, extra keys, followups with no
...
**Measured result: 57/58 fields (98%), 0 hard failures.**
...
tests/             75 deterministic tests (incl. a selector-stability source scan)
