...
- **Changing your mind works.** `setSex()` rewrites the answers, not just the meta —
  switching from female to male *nulls* `menstrual_cycle`, so a stale value can never
...
- **Q4 / Q10 "none of these".** These two have no `none` option in the schema, but an
  ...
  **UI-only** button that writes `[]` and marks the step answered. The output stays
  exactly on-schema; the patient still has a way to say "nothing" instead of guessing.
...
**`npm test` — 75 deterministic tests, no API key needed.** These are the dependable
checks: the step builder, sex gating in all four states, every conditional-followup
rule in both directions, exclusive options, 16-key coverage, and — most of the value —
the slice layer fed what a 70B model *actually* returns: markdown fences, prose
...
**Result: 57/58 fields (98%), 0 hard failures.** The eval paid for itself immediately —
its first run caught the model writing `Other: done = false` for a procedures row the
...
1. **shadcn/ui primitives are hand-written** (`components/ui/Button.tsx`) rather than
...
2. **Q6 suggests `"Menopausal"`, not `"Not applicable"`, and suggests rather than
   pre-fills.** Reasoning above.
3. **Q4 gets "Not sure" and Q10 gets "None of these"** as UI-only affordances writing
   `[]`. No schema option was invented.
4. **`NVIDIA_MODEL` is `openai/gpt-oss-20b`, not a Llama 70B.** Not a preference — the
   brief's suggestion is retired (410 Gone). Reasoning and latency notes above.
5. **Not done by me:** the Vercel deploy, any git/GitHub step (you asked me not to
