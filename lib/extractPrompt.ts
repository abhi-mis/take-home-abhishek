...
export const SYSTEM_PROMPT = `You extract structured intake answers from a patient's spoken reply at a hair clinic in India.
Input: (1) the JSON schema for ONE question, (2) the transcript.
The transcript may be English, Hindi, or Hinglish (mixed) and may be loosely transcribed.
...
/**
 * Model + sampling settings, in one place so the route and the eval script cannot
 * drift apart (a benchmark that runs different settings than production is worthless).
 *
 * `reasoning_effort` matters a lot here. The current NVIDIA catalog is mostly
 * reasoning models, and on a patient-facing screen their default effort is fatal:
 * measured on this account, gpt-oss-120b took 94-120s per call while gpt-oss-20b at
 * effort "low" answered the same transcript identically in 4.7s. Extraction against a
 * fixed 8-field schema needs no deliberation, so low effort costs nothing and is the
 * difference between a usable and an unusable step.
 *
 * The SDK's published types lag the catalog's accepted fields, so the extra key is
 * attached through a narrow cast rather than `any`.
 */
...
export function modelConfig(): ModelConfig {
  const effort = process.env.NVIDIA_REASONING_EFFORT ?? "low";
  return {
    model: process.env.NVIDIA_MODEL ?? "openai/gpt-oss-20b",
    temperature: 0, // deterministic: the same transcript must fill the same fields
    max_tokens: 900, // enough that a fully-populated 5-row table is never truncated
...
/**
 * NIM models are not guaranteed to return bare JSON, so strip code fences and take
 * the outermost object before parsing. Returns null rather than throwing.
 */
